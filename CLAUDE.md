...
- The release workflow's `Browser test` step runs `tests/e2e/interactivity.mjs` (Playwright) against the built image with Unraid settings: it signs up, checks the theme toggle and account menu respond, adds a category, payee, accounts and a transfer, quick-adds a transaction, downloads and restores a backup, then signs up a second user, shares the ledger with them (the invitation must pop up live), and checks the shared transaction is tagged, read-only, and hidden when the layer is switched off. Extend it when adding features that only work interactively.
...
- Unraid Community Apps lists Production, but the template lives in a **separate repository**: [`007darkmatter5/unraid-templates`](https://github.com/007darkmatter5/unraid-templates) (`templates/ledgerly.xml` there, alongside MeshVault's). CA wants one repo per maintainer, not per app, so this repo holds no `ca_profile.xml` or template. Editing the template means a commit in that repo; CA re-reads it on its own schedule. Keep its paths, ports and variables in step with the Dockerfile, and its Overview describing **Production** (`main`), not whatever `beta` has added. The icon and screenshots stay here in `docs/images` and are referenced by raw `main` URLs. Docs, `LICENSE` and `docs/**` changes don't trigger a release (`paths-ignore`).
...
**Layers.**
- `Finance/` is pure and unit-tested, with no EF and no DI: `Recurrence` (dates), `Projection` (running balance), `BillSchedule` (due dates merged with recorded payments), `TransferSchedule` (the same for transfers), `Amortization`/`LoanMath`/`LoanPayments` (loans). Put new money logic here and test it.
...
- App-wide singletons: `AppSettingsStore` (key/value `AppSettings` table), `RegistrationPolicy`, `EmailSettingsStore`, `SmtpEmailService`, `SharingNotifier`.
...
- Every `LedgerService` write must be limited to the active ledger id, and must also validate referenced ids (a bill's account/loan, an income's account) with `EnsureInLedgerAsync`, which checks the *active* ledger, never the shared ones, so nothing can point across ledgers. Reads default to the active ledger too; add a case to `LedgerServiceTests` (in-memory SQLite) for any new entity, read or write path.
- **Sharing (phase 1, view only).** An owner shares their personal ledger (never the sample) with an existing account by email (`ShareAsync`); a `LedgerMember` row is an invitation until the recipient accepts (`AcceptedAt`). The recipient's layer settings (`IsVisible`, `Color`, `Nickname`) live on that row. `GetScopeAsync` builds a `LedgerScope` fresh on every call (home ledger + accepted shared ledgers; shared ones are hidden while the home ledger is the sample), so removed access or a layer switched off applies immediately. Read methods take `includeShared: true` to add the visible shared ledgers (`ReadLedgerIdsAsync`); only pages that *display* data pass it. Pickers in dialogs and the Categories/Payees pages stay on the user's own ledger, or saving would hit another ledger's ids. `LedgerRole` has Viewer/Contributor/Editor, but only Viewer exists in the UI: writes don't look at roles yet, they simply never leave the home ledger.
- `MainLayout` cascades the `LedgerScope` (and a `RefreshLedgers` callback) to pages. Pages put `<LedgerTag LedgerId=…/>` (a lock in the ledger's color, named in its tooltip and aria-label) right after the item's name, and use `Scope.RowStyle` (a colored left edge) and `Scope.IsHome(...)` to tag shared rows and hide edit controls on them; action lists (overdue, not marked paid, missed transfers, unassigned bills) count only the user's own. Category and payee filters match by name across ledgers (`NameFilter`). `SharingNotifier` (singleton, in-memory) tells open sessions when an invitation arrives or access changes, so the prompt and the Ledgers menu update without a reload.
...
- Admins manage app settings and roles only; they can't read other users' ledgers in the app (but can download a whole-install backup, see below). Being an admin gives no access to shared ledgers either; only a `LedgerMember` row does.
...
- Payees work the same way (`Payee`: name unique NOCASE per ledger, optional `WebsiteUrl`, notes). `Bill.PayeeId` and `Loan.LenderId` both point at `Payee`, so a lender's website lives on the payee; deleting a payee nulls both. Dialogs pick one with `Shared/PayeePicker` (its + button creates a payee inline). Website URLs go through `WebLinks.TryNormalize` (http/https only) and render with `ExternalLink`.
- Migrations that replace a column with a relation must copy data with `migrationBuilder.Sql` before the drop. EF's SQLite table rebuilds run where the rebuilding operation appears, so order the `Up` operations explicitly (see `BillCategories`) and check `dotnet ef migrations script`. Table-qualify every column in raw SQL subqueries: an unqualified `"Name"` silently binds to the inner table (see `Payees`).
- The Running Balance page remembers its accounts, range and worst-case toggle on the active `Ledger` (`SaveProjectionViewAsync`), so each ledger keeps its own; a `?accounts=` link overrides it for that visit without being saved.
- Credit cards are `Account`s with `Type = CreditCard` (balance stored negative; the UI shows "amount owed" as positive) plus card fields (limit, APR, statement day, monthly spending, last statement balance, minimum payment settings) that `SaveAccountAsync` clears for other types. Bills charged to a card have it as `PayFromAccountId`; a bill that pays a card has `CardAccountId` and a `CardPaymentRule` (statement in full, minimum, fixed amount), must be paid from a non-card account, and can't also pay a loan.
- `Finance/CreditCards.Simulate` plays a card forward (charges, prorated monthly spending and interest at each statement close, payments by rule unless an amount is recorded). Interest is an estimate: monthly APR/12 on the balance at close when the previous statement wasn't paid off. `Projection.Build` uses it for card payment amounts (entries between two projected accounts are `IsTransfer` and excluded from totals) and adds card spending/interest/payment entries when a card is projected. `LedgerService.GetBillsAsync` fills the unmapped `Bill.PaymentEstimates`, which `BillDue.Amount` uses, so schedules, dashboards and Payments show estimated card payments.
- A `Transfer` (`/transfers`) moves money between two of the ledger's own accounts on a recurrence, with `TransferOccurrence` recording the actual amount and the day it moved for one scheduled date (`TransferSchedule` merges the two, like `BillSchedule`). `Projection.Build` adds an entry to whichever end is projected; when both are, both legs are `IsTransfer` and drop out of the money in/out totals. `SaveTransferAsync` rejects a blank name, a negative amount, the same account at both ends, and a credit card at either end — a card is paid by a bill with a `CardAccountId` so `CreditCards.Simulate` still owns its balance. `IsAutomatic` is the autopay equivalent: scheduled dates count without being marked, so `TransferDue.IsMissed` never flags them. Deleting either account deletes the transfer (it needs both ends).
- A `Transaction` (`/transactions`, and the app bar's quick-add button) is a single purchase or deposit that isn't a bill: `Amount` is always positive and `Direction` (Spent/Received) gives the sign. Like everything else it only moves a projection when dated on or after its account's `BalanceAsOf` (earlier ones are history). It can be on a credit card: `CreditCards.Simulate` charges it on its date and counts net logged spending toward the card's `MonthlySpending` for that statement cycle, so each cycle adds the larger of the estimate and what was logged. Because card payment estimates depend on this, `Projection.Build`, `Simulate` and `EstimatePayments` take transactions as a required argument; pass them everywhere. The Payments history includes spent transactions (up to today) next to bill payments, and the bill calendar shows all of them. Deleting the account deletes its transactions; deleting a category or payee nulls it. Loan payments stay bills (`LoanPayments` reads bills).
- Payment history (`PaymentHistory`, `/payments`) is built from `BillOccurrence.PaidOn`, so it includes inactive bills; unmarked autopay due dates of active bills count as paid on the due date (`IsAssumed`). "Not marked paid" uses the same balance-date cutoff as overdue. `LedgerService.MarkPaidAsync` bulk-marks due dates paid without overwriting recorded amounts, notes or an existing paid date. Deleting a bill deletes its history.
